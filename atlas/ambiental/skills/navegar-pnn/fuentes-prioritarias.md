...
| `ecoturismo/#estadisticas_visitantes` | Las estadísticas de visitantes son anclas en la página de Ecoturismo, no URLs independientes. El archivo histórico antes de 2022 puede estar en `old.parquesnacionales.gov.co/portal/es/ecoturismo/`. |
| `cert.parquesnacionales.gov.co` | Herramienta administrativa para certificaciones SINAP. Acceso público solo a noticias y manuales de usuario (enlazados a Google Sites). |
| `ue.parquesnacionales.gov.co` | Portal de cooperación con la Unión Europea. Devuelve HTTP 403. Sin acceso verificado. |
