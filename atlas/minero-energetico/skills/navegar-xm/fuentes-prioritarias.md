...
|---|---|---|---|
| API pública XM (SiNERGOX) | `servapibi.xm.com.co` | REST API pública sin autenticación; método POST; endpoints `/hourly`, `/daily`, `/monthly`, `/lists`; cubre Hidrología, Demanda, Oferta y Generación, Transacciones y Precios, Intercambios Internacionales, Emisiones CO2 | 2026-05-30 |
...
| SIMEM | `simem.co` | Sistema de Información del Mercado Eléctrico Mayorista; portal de datos abiertos operado por UPME y XM; API propia en `simem.co/backend-files/api/PublicData?datasetid={}` | 2026-05-30 |
...
| `simem.co` | Portal de datos abiertos del sector; API: `GET simem.co/backend-files/api/PublicData?datasetid={ID}`. Restricciones: horaria/diaria máx 31 días, semanal/mensual máx 731 días, anual máx 1827 días |
